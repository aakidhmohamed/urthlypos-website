...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      <nav className="fixed top-8 inset-x-0 z-50 flex justify-center px-4 pointer-events-none">
        <div className={`pointer-events-auto w-full max-w-5xl bg-surface-base/80 backdrop-blur-xl border border-border-subtle rounded-full shadow-premium pl-4 pr-2 py-2 flex items-center justify-between transition-all duration-500 ease-premium ${scrolled ? 'bg-surface-base/90 border-border shadow-black/50' : ''}`}>
          ...
          >
            <div className="bg-primary/10 border border-border-premium p-0.5 rounded-full group-hover:border-primary/40 transition-all duration-300 overflow-hidden">
            ...
            <span className="font-heading font-bold text-lg text-text tracking-tight group-hover:text-white transition-colors">
              Urthly<span className="font-normal text-text-muted">POS</span>
            </span>
          </button>

          {/* Center Navigation Container */}
          <div className="hidden md:flex items-center bg-white/5 border border-border-subtle rounded-full p-1 shadow-inner gap-1">
            {NAV_ITEMS.map((item) => (
              <a
                key={item.label}
                href={item.href}
