        ...
        <div className="text-center max-w-4xl mx-auto mb-16">
          <FadeIn delay={0}>
            ...
            <Parallax speed={1.2} maxOffset={20} className="inline-block">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-surface border border-border text-primary text-xs font-bold mb-8 tracking-wide uppercase font-mono animate-float hover:scale-105 transition-transform duration-300 ease-premium cursor-default">
          ...
          <FadeIn delay={100}>
            <h1 className="text-5xl md:text-7xl font-heading font-extrabold text-text tracking-tight mb-8 leading-[1.1]">
              Stop Losing Sales When <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-green-400 to-white">Your Internet Dies</span>
            </h1>
          </FadeIn>
          
          <FadeIn delay={200}>
            <p className="text-lg text-text-muted mb-10 max-w-2xl mx-auto font-normal leading-relaxed">
              Frustrated with slow cloud POS systems, spinning loading screens, and monthly fees? 
              Switch to the <strong>offline-first solution</strong> that guarantees 100% uptime, instant billing, and zero recurring costs.
            </p>
          </FadeIn>

          <FadeIn delay={300}>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 max-w-md mx-auto">
              <button className="group w-full sm:w-auto whitespace-nowrap px-8 py-4 bg-primary hover:bg-primary-hover text-white rounded-full font-heading font-bold transition-all duration-300 ease-premium flex items-center justify-center gap-2 shadow-glow hover:shadow-glow-lg transform hover:scale-105 hover:-translate-y-1 active:scale-95 active:translate-y-0 focus-ring">
                <Video className="w-5 h-5 transition-transform group-hover:translate-y-0.5" />
                Book Free Demo
              </button>
              <button className="w-full sm:w-auto whitespace-nowrap px-8 py-4 bg-surface hover:bg-zinc-800 text-text border border-border rounded-full font-heading font-bold transition-all duration-300 ease-premium flex items-center justify-center gap-2 hover:border-[#25D366]/50 hover:text-[#25D366] focus-ring hover:scale-105 active:scale-95">
                    ...
                    <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-white/[0.02]">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 shadow-[0_0_10px_rgba(16,185,129,0.2)]">
                                ...
                                <span className="text-[10px] font-mono font-bold text-emerald-500 tracking-wider">LIVE_SYNC</span>
                            </div>
                    ...
                    <div className="flex flex-col md:flex-row min-h-[500px] md:min-h-[600px]">
                        ...
                        <div className="flex-1 p-6 md:p-8 bg-gradient-to-br from-transparent to-black/40">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 mb-8">
                                {[
                                    { label: 'Total Revenue', value: '$12,450.00', trend: '+15%', icon: DollarSign, color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
                                        ...
                                        {[40, 65, 45, 80, 55, 90, 70, 85, 60, 75, 50, 95].map((h, i) => (
                                            <div key={i} className="w-full relative group/bar h-full flex items-end">
                                                ...
                                                <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-zinc-900 border border-white/10 text-white text-[10px] py-1.5 px-3 rounded-lg opacity-0 group-hover/bar:opacity-100 transition-all duration-300 transform translate-y-2 group-hover/bar:translate-y-0 shadow-xl whitespace-nowrap z-20 pointer-events-none font-mono">
                                                    <span className="text-primary font-bold block mb-0.5">${h * 150}</span>
                                    ...
                                    <div className="flex-1 space-y-3 overflow-hidden relative">
                                        {[1, 2, 3, 4].map((_, i) => (
                                            <div key={i} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/5 transition-colors cursor-pointer border border-transparent hover:border-white/5 group/item">
                                                ...
                                                <div className="text-xs font-mono text-primary bg-primary/10 px-2 py-1 rounded border border-primary/10">+$124</div>
                                            </div>
                                        ))}
                                        {/* Fade out bottom */}
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-4 border-t border-white/5 text-[10px] md:text-xs">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.5)]"></div>
                                        <span className="text-text-muted font-medium">Sync: <span className="text-emerald-500">Auto (12ms)</span></span>
                                    </div>
                                </div>
